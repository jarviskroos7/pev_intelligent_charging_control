...
class Normal_env():
    def __init__(self):
        self.voltage = 240
        self.battery_volume = 80 * 1000 / self.voltage  # amp
        self.resistance = 0
        ...
        self.charge_interval = 12

        self.delta_soc_interval = 0.01
    ...
    def step(self,state, action):
        ...
        if action == 2:
            delta_soc =  np.round(self.I_max * self.charge_interval /60 /self.battery_volume*self.loss_coefficient,2)
            new_state[0] = state[0] - delta_soc

        elif action == 0:
            delta_soc = np.round(self.I_max * self.charge_interval / 60 / self.battery_volume * self.loss_coefficient,
                                 2)
            new_state[0] = state[0] + delta_soc
        else:
            new_state[0] = state[0]

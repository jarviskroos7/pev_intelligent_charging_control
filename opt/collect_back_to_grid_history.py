...
print(len(data))
session_pair = []
back_to_grid_agent_emission_list = []
...
for i in range(len(data)):
    ...
    if month >= 10 or month <= 3:
        ...
        season = "summer"
    back_to_grid_emission, back_to_grid_charging_history = back_to_grid_agent.get_total_emission_value(start_step, end_step, start_soc, end_soc, season)
    back_to_grid_agent_emission_list.append(back_to_grid_emission)

    with open('back_to_grid_charge_history_%s.txt'% back_to_grid_agent.R, 'a') as f:
        f.write("%s\n "%back_to_grid_charging_history)

    with open('back_to_grid_charge_volume_%s.txt'% back_to_grid_agent.R, 'a') as f:
        f.write("%s\n "%back_to_grid_emission)